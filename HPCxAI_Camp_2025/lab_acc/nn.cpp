...
void LinearLayer(float *restrict A, float *restrict B, float *restrict C, float *restrict D, int n, int k, int m) {
    #pragma acc data copyin(A[0:n*k], B[0:k*m], C[0:m]) copyout(D[0:n*m])
    {
        // Parallel loop to compute D = A * B + C
...
/* TODO: Parallel the for loops */
void Sigmoid(float *restrict A, int n, int m) {
    #pragma acc data copy(A[0:n*m])
    {
        // Parallel loop to compute Sigmoid
        #pragma acc parallel loop collapse(2) gang vector
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
 ...
 */
 ...
 /* TODO: Parallel the for loops */
void Argmax(float *restrict A, int *restrict D, int n, int m) {
