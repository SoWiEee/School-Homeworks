...
public:
    ...
    void handleCustomer(shared_ptr<Customer> c) {
        ...
        if (!queue_.empty() && queue_.front().get() == c.get()) {
            ...
            if (plan) {
                applyPlanLocked(*c, *plan, logicalTime_.load());
                queue_.pop_front();
                pushLogLocked(formatLogLocked(*c, logicalTime_.load(), "seated", c->seatIDs));

                // 計算離席時間，登記到 timeHeap
                c->leaveTime = c->seatTime + c->EstDiningTime;
                timeHeap_.push(c->leaveTime);

                // 通知 clock/thread 有新的事件
                cvClock_.notify_one();
                cv_.notify_all();
            } else {
            ...
            // 否則讓 wait queue 後面的人嘗試入座
            for(int i = 1; i < queue_.size(); i++ ){
                if(!queue_.empty() && queue_[i]){
                    auto plan = makePlanLocked(*c);
                    if (plan) {
                        applyPlanLocked(*c, *plan, logicalTime_.load());
                        queue_.erase(queue_.begin()+i);     // delete customer
                        pushLogLocked(formatLogLocked(*c, logicalTime_.load(), "seated", c->seatIDs));
        ...
        while (!c->seated) {
            cv_.wait(lk, [&]{
                ...
                // 只有 head 可以嘗試
                if (!queue_.empty() && queue_.front().get() == c.get()) {
                    auto plan2 = makePlanLocked(*c);
                    if (plan2) {
                        applyPlanLocked(*c, *plan2, logicalTime_.load());
                        queue_.pop_front();
                        pushLogLocked(formatLogLocked(*c, logicalTime_.load(), "seated", c->seatIDs));
                        c->leaveTime = c->seatTime + c->EstDiningTime;
                        timeHeap_.push(c->leaveTime);
