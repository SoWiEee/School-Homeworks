...
struct Snapshot {
    ...
    int B = 0, W = 0;
    vector<int> singlesOcc; // size 14, 0 free else custID
    vector<int> sofa4Occ;   // size 5
    vector<int> sofa6Occ;   // size 3
...
class SushiBar {
public:
    SushiBar(int singles = 14, int four = 5, int six = 3, int baby = 3, int wheel = 2)
    ...
    bool trySeatOneLocked(int now) {
        ...
        const bool headBlocksAuxBypass = (head->BabyChair >= 3) || (head->WheelChair >= 2);
        for (auto it = next(queue_.begin()); it != queue_.end(); ++it) {
            auto cand = *it;
            if (!cand || cand->seated) continue;

            // If the head needs many baby chairs / wheelchair spots, do not let customers who also
            // need baby chairs or wheelchair spots bypass. This prevents consuming scarce auxiliary
            // resources while the head is already blocked on them.
            // if (headBlocksAuxBypass && (cand->BabyChair > 0 || cand->WheelChair > 0)) {
            //     continue;
            // }
    ...
    // 取得完整 log（用於大量測資時關閉 TUI、結束後一次輸出）
    vector<string> getAllLogsSnapshot() const {
        lock_guard<mutex> lk(mu_);
        return allLogs_;
    }
    ...
    optional<AllocationPlan> makePlanLocked(const Customer& c) const {
        ...
        switch (c.Type) {
            ...
            case CustomerType::FAMILY: {
                ...
                if (c.PartySize == 6) {
                    if (auto idxs = findContinuousSinglesLocked(6)) {
                        plan.kind = PlanKind::Singles;
                        plan.singleIdx = *idxs;
                        plan.seatIDs.clear();
                        for (int i : *idxs) plan.seatIDs.push_back(fmtID("S", i + 1, 2));
    ...
    void uiLoop() {
        while (!uiStop_.load()) {
            Snapshot snap;
            {
                ...
                cv_.wait_for(lk, chrono::milliseconds(100));
                snap = snapshotLocked();
            }
            ...
            // 4P
            cout << "4P Sofas (4P01..4P05; 4P05 accessible):\n  ";
            ...
            // 6P
            cout << "6P Sofas (6P01..6P03; 6P03 accessible):\n  ";
...
int main(int argc, char** argv) {
    ...
    // 大量測資時不啟動即時座位 TUI，改為結束後一次輸出完整 log
    static constexpr int kLogOnlyThreshold = 50;
    const int totalGroups = (int)(incomingCustomers.size() + initialCustomers.size());
    const bool logOnlyMode = (totalGroups >= kLogOnlyThreshold);
    if (!logOnlyMode) {
        bar.startUI();
    ...
    if (logOnlyMode) {
        cout << "=== Full Log ===\n";
        auto logs = bar.getAllLogsSnapshot();
