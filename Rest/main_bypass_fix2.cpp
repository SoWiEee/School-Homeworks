// g++ -std=c++17 -O3 -pthread main_bypass_fix2.cpp -o sushi.exe
// [Fix 1] Line 244, preloaded cust. also increase finishedCount_
// [Fix 2] Line 800, correct log order (UX improvement)
...
public:
    ...
    bool trySeatOneLocked(int now) {
        ...
        for (auto it = next(queue_.begin()); it != queue_.end(); ++it) {
            ...
            // pushLogLocked(*head, now, "bypassed", {});
