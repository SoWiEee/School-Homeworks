// g++ -std=c++17 -O3 -pthread main_bypass.cpp -o sushi2.exe -fexec-charset=big5
...
public:
    ...
    void preload(const vector<shared_ptr<Customer>>& initialCustomers) {
        ...
        for (auto& c : initialCustomers) {
            ...
            if (!plan) {
                pushLogLocked(formatLogLocked(*c, now, "PRELOAD_FAILED", {}));
                continue;
            }
            // 扣資源 + 佔位
            applyPlanLocked(*c, *plan, now);
            pushLogLocked(formatLogLocked(*c, now, "PRE-SEATED", c->seatIDs));
    ...
    void handlePreloaded(shared_ptr<Customer> c) {
        ...
        // release rsrc
        releaseLocked(*c);
        pushLogLocked(formatLogLocked(*c, logicalTime_.load(), "RELEASE", c->seatIDs));
        // 通知大家可能有人可以入座了
        cv_.notify_all();
        cvClock_.notify_one();
    }
    ...
    void handleCustomer(shared_ptr<Customer> c) {
        ...
        if (!isEverSatisfiableLocked(*c)) {
            pushLogLocked(formatLogLocked(*c, logicalTime_.load(), "rejected", {}));
            finishedCount_++;
            cv_.notify_all();
            cvClock_.notify_one();
            return;
        ...
        // - If head is blocking, we may seat a non-head
        if (!queue_.empty() && queue_.front().get() == c.get()) {
            if (!trySeatOneLocked(logicalTime_.load())) {
                pushLogLocked(formatLogLocked(*c, logicalTime_.load(), "waited", {}));
            } else if (!c->seated) {
                // Someone else might have been seated
                pushLogLocked(formatLogLocked(*c, logicalTime_.load(), "waited", {}));
            }
        } else {
            // Not head => wait.
    ...
    // - If head can be seated now => seat head
    // - If head cannot be seated now:
    //     * If head.skipCount >= kMaxBypass => seat nobody until head becomes feasible.
    ...
    bool trySeatOneLocked(int now) {
        ...
        if (head->skipCount >= kMaxBypass) {
            ...
            queue_.pop_front();
            pushLogLocked(formatLogLocked(*head, now, "seated", head->seatIDs));
        ...
        for (auto it = next(queue_.begin()); it != queue_.end(); ++it) {
            ...
            queue_.erase(it);
            pushLogLocked(formatLogLocked(*cand, now, "seated", cand->seatIDs));

            cand->leaveTime = cand->seatTime + cand->EstDiningTime;
            timeHeap_.push(cand->leaveTime);

            head->skipCount++;
            // pushLogLocked(formatLogLocked(*head, now, "bypassed", {}));
            pushLogLocked(formatLogLocked(*head, now, "waited", {}));
...
private:
    ...
    void uiLoop() {
        while (!uiStop_.load()) {
            ...
            {
                unique_lock<mutex> lk(mu_);
                // wait either a notify, or timeout 0.5s
                cv_.wait_for(lk, chrono::milliseconds(500));
                snap = snapshotLocked();
            }
...
int main(int argc, char** argv) {
    ...
    // 依 arrival time，再依 id 排序，並給定 ArrivalSeq
    ...
    // wait only arriving customers
    bar.waitAll((int)incomingCustomers.size());
