...
參數（固定）：gaussian_ksize=5, adaptive_block=71, C=2, morph_ksize=5, morph_iter=1, Hough param2=32

param2 由 YOLO 輔助診斷 + GT XML param2 掃描（25–40）決定：28→32 使 RBC 平均誤差從 36.9% 降至 28.1%。

| 影像 | RBC pred/GT | err% | WBC pred/GT | err% | PLT pred/GT | err% |
|------|------------|------|------------|------|------------|------|
| BloodImage_00001 | 12/18 | 33.3% ✗ | **1/1** | **0%** ✓ | 1/0 | — |
| BloodImage_00002 | 18/15 | 20.0% ✓ | **1/1** | **0%** ✓ | 1/0 | — |
| BloodImage_00003 | 11/15 | 26.7% ✓ | **1/1** | **0%** ✓ | **1/1** | **0%** ✓ |
| BloodImage_00004 | 14/11 | 27.3% ✓ | **1/1** | **0%** ✓ | 0/1 | 100% ✗ |
| BloodImage_00005 | 12/18 | 33.3% ✗ | **1/1** | **0%** ✓ | 1/3 | 66.7% ✗ |
| **平均** | — | **28.1%** | — | **0.0%** | — | **33.3%** |

**備註**：
- WBC：5/5 全數正確（0% 平均誤差），HSV 紫色遮罩 + 凸包方法穩定有效。
- RBC：00001 和 00005 稍微低估（GT 為稀疏標注，實際可見細胞更多）；00002 由 86.7% 大幅降至 20%。
- Platelet GT=0 的圖（00001、00002），評估函式 gt=0 時回傳 0%，實際各有 1 個誤報，平均誤差 33.3% 為保守估計。

---

## 六、YOLO 輔助調參結論

- YOLO fine-tune（YOLOv8n, 25 epochs）用於診斷 Hough 過偵測原因
- TP/FP 顏色特徵差異 < 0.1（S、V、A、B、rbc_ratio 幾乎相同），顏色濾波無效
- param2 掃描（GT XML 為參考）：param2=32 最佳，平均誤差 28.1%
- 改善效果：00002 86.7%→20%，00004 54.5%→27.3%

## 七、待改進

1. **Platelet 漏偵測**：00004 和 00005 有漏偵；血小板染色較淡且小，HSV 閾值較保守。
2. **GT=0 的誤差計算**：評估函式以 gt=0 時回傳 0%，掩蓋血小板假陽性。
