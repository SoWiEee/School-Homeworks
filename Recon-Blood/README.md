...
| RBC | 0.78 | 0.78 | 0.79 | +9.2% |
| Platelet | 0.72 | 0.73 | 0.70 | −6.1% |
| **micro** | **0.79** | **0.79** | **0.80** | — |

WBC 在 IoU 0.3 下 F1 達 ~0.93（精確率 0.94–0.96），RBC 召回率 ~0.82、F1 ~0.79，三類細胞計數誤差皆在 **±30%** 內，且 test/val/全資料集一致（未過擬合）。經框大小校正後 **IoU 0.5 下 micro F1 亦達 ~0.76**（WBC 0.87、RBC 0.76），0.3 與 0.5 落差僅 ~0.03，框不靠寬鬆閾值也夠準。完整優化歷程（含偏紫抹片的整張圖白血球框修正、框大小校正、包含抑制過度修正）見 [`report.md`](report.md)。
