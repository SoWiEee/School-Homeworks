...
**WBC（`detect_wbc`）**
**violet-hue 遮罩**（hue 120–158、S≥70、V<232、A>132、B<128；`wbc_violet_mask`）→ 形態學**開運算**取出實心核 → 小幅膨脹只合併同一核的分葉 → 連通元件 → 幾何（夠大 > 1.05·r0）規則 → 中心距離 NMS。框取核心外接框（GT ≈ 1.03×核外接框，只加極小 padding）。用窄 hue 鎖定真紫色，連淡染核都抓得到，又不會誤收偏粉紅的 RBC 團。**偵測完的 WBC 框會傳給 RBC 與血小板偵測器，排除中心落在 WBC 框內的候選**，白血球就不會被切成紅血球或血小板。
...
| --- | --- | --- | --- | --- |
| WBC | 0.89 | 0.88 | 0.86 | +12.0% |
| RBC | 0.71 | 0.73 | 0.73 | −9.1% |
| Platelet | 0.73 | 0.73 | 0.70 | −4.1% |
| **micro** | **0.73** | **0.74** | **0.74** | — |

WBC 在 IoU 0.3 下 F1 達 ~0.88（召回率 0.93–0.95），三類細胞計數誤差皆在 **±30%** 內，且 test/val/全資料集一致（未過擬合）。完整優化歷程見 [`report.md`](report.md)。
