...
| Platelet | 0.72 | 0.78 | 0.71 | +22.4% |
| **micro** | **0.82** | **0.81** | **0.82** | — |

WBC 在 IoU 0.3 下 F1 達 ~0.95（精確率 ~0.98，經修正 14 的相對飽和度／尺寸閘把假陽性 77→28、召回不變），RBC 經 Hough 圓偵測補強、自適應紫色排除與修正 15 的尺寸感知去重後精確率 0.75→0.78、F1 ~0.81（計數過預測由 +16.9% 收斂到 +9.5%），三類細胞計數誤差皆在 **±30%** 內，且 test/val/全資料集一致（未過擬合）。**IoU 0.5 下 micro F1 亦達 ~0.79**（WBC 0.89、RBC 0.78），0.3 與 0.5 落差僅 ~0.03，框不靠寬鬆閾值也夠準。完整優化歷程（含偏紫抹片白血球框、框大小校正、Watershed 前景修正、Hough 相連細胞補強、自適應紫色排除、白血球假陽性清除與紅血球團塊去重）見 [`report.md`](report.md)。
...
完整 15 項修正歷程與資料驅動的判斷（含「為什麼不那樣做」）見 [`report.md`](report.md)。
