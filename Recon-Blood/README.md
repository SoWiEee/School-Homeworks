...
**RBC（`detect_rbc_rule`）**
取前處理後的二值圖 → `findContours`（`RETR_LIST`）抽輪廓 → 用**面積、邊界框尺寸、長寬比、圓形度**四道規則篩選（圓形度 `4πA/P²`），並排除落在紫色區內的輪廓（避免把 WBC 算成 RBC）→ **包含抑制**（當更細的單顆框中心落在某團塊框的**中央 80%** 內，丟掉該團塊框，避免一顆細胞被框兩次；用中央 80% 而非整框，以免邊緣相鄰的獨立細胞被誤刪）→ 中心距離 NMS。`RETR_LIST` 的內孔輪廓本身即可把黏連團塊切成單顆（保留召回）；因 RBC 大小極一致（GT 邊長 ≈ 2·r0），框半徑往 `r0` 收斂、再 **×1.13 校正**（暗環輪廓比 GT 框小 ~11%，補回後 IoU 0.5 明顯提升）以提升 IoU。接著用 **Hough 圓偵測**（`hough_rbc_candidates`，固定半徑先驗）補回「相連／環不完整、輪廓抓不到」的紅血球，召回率 0.82→0.86、F1 0.79→0.80（見 [`report.md`](report.md) 修正 11）。最後加一道**尺寸感知去重**（中心半徑 `1.0·r0`，優先保留邊長最接近標準 `2·r0` 的框）：輪廓＋Hough 在密集團塊的細胞縫隙會過度偵測、生出壓在真細胞上的重複框（占假陽性 60%），此去重把它們併掉而保留真細胞框，精確率 0.75→0.78、F1 0.805→0.811、計數過預測 +16.9%→+9.5%（修正 15）。

**血小板（`detect_platelets_rule`，純規則）**
寬鬆紫色遮罩取小面積連通元件 → 抽取手工特徵 → **雙分支接受規則**：（A）強染顆粒——用**顆粒紋理**（飽和度變異 `S_std`、灰階變異 `gray_std`）作主判據，配合顏色與面積門檻，並收緊兩道精確率閘（**深紫 `B_mean≤110`、圓形 `circ≥0.30`**，擋掉偏藍、破碎的雜斑，見 report.md「修正 16」）；（B）淡染但圓——染色較淡（`S_mean≥60`）但形狀夠圓（`circ≥0.68`）且偏紫（`B_mean≤112`）的真血小板（救回約 ¼ 漏抓的淡染血小板，見 report.md「修正 12」）→ 任一分支通過即收 → 排除中心落在**加 0.10·r0 margin 的 WBC 框**內的候選（去掉白血球邊緣的假血小板）。真血小板內部有顆粒造成明顯亮度起伏、整體深紫且圓，背景雜訊則偏藍、破碎或均勻——紋理＋顏色＋圓形度三軸是最乾淨的分界。框設成固定大小以對齊一致的血小板 GT。精確率閘使血小板 P 0.69→0.80、F1 0.72→0.76，且召回維持 0.72（P/R/F1 全 >0.70）。
...
| --- | --- | --- | --- | --- |
| WBC | 0.96 | 0.95 | 0.95 | −5.3% |
| RBC | 0.81 | 0.80 | 0.81 | +9.9% |
| Platelet | 0.77 | 0.80 | 0.76 | +6.1% |
| **micro** | **0.82** | **0.81** | **0.82** | — |

WBC 在 IoU 0.3 下 F1 達 ~0.95（精確率 ~0.98，經修正 14 的相對飽和度／尺寸閘把假陽性 77→28、召回不變），RBC 經 Hough 圓偵測補強、自適應紫色排除與修正 15 的尺寸感知去重後精確率 0.75→0.78、F1 ~0.81（計數過預測由 +16.9% 收斂到 +9.5%），血小板經修正 16 的「深紫＋圓形精確率閘」後精確率 0.69→0.80、F1 0.72→0.76（P/R/F1 全數突破 0.70），三類細胞計數誤差皆在 **±30%** 內，且 test/val/全資料集一致（未過擬合）。**IoU 0.5 下 micro F1 亦達 ~0.79**（WBC 0.89、RBC 0.78），0.3 與 0.5 落差僅 ~0.03，框不靠寬鬆閾值也夠準。完整優化歷程（含偏紫抹片白血球框、框大小校正、Watershed 前景修正、Hough 相連細胞補強、自適應紫色排除、白血球假陽性清除、紅血球團塊去重與血小板精確率閘）見 [`report.md`](report.md)。
...
| `case15_rbc_dedup_b5aa7a10.png` | 修正 15 | 密集紅血球團塊：**前**輪廓＋Hough 在細胞縫隙過度偵測（多餘重複框、12 FP）；**後**尺寸感知去重併掉偏移的縫隙框（8 FP，真細胞 13/13 全保留） |
| `case16_platelet_fp_ad4282bd.png` | 修正 16 | **前**一顆偏紅、破碎的紅血球邊緣雜斑被誤判成血小板（紅框、1 FP）；**後**深紫＋圓形精確率閘擋掉雜斑（0 FP），左上真正深紫的血小板仍保留 |
...
![case15](demo/cases/case15_rbc_dedup_b5aa7a10.png)
![case16](demo/cases/case16_platelet_fp_ad4282bd.png)

完整 16 項修正歷程與資料驅動的判斷（含「為什麼不那樣做」）見 [`report.md`](report.md)。
