...
- `purple_mask_strict()`：高精確率（B≤115 只保留深紫），用於「把 RBC 排除在紫色區外」與 Watershed／視覺化。
- `purple_mask_loose()`：高召回率，用於血小板候選抽取（寧可多抓再用規則過濾）。

### 三種細胞偵測策略（WBC 最先執行，其框用來排除 RBC/血小板）

**WBC（`detect_wbc`）**
**violet-hue 遮罩**（hue 120–158、S≥70、V<232、A>132、B<128；`wbc_violet_mask`）→ 形態學**開運算**取出實心核 → 小幅膨脹只合併同一核的分葉 → 連通元件 → 幾何（夠大 > 1.05·r0）規則 → **核心深染驗證**（核心區飽和度中位數 ≥ 100 或 LAB-B ≤ 102）→ **過大候選的相對飽和度閘**（邊長 ≥ 4.5·r0 者，核心飽和度須高出整張影像中位數 ≥ 70，否則視為整張偏紫的染色假象而丟棄）→ 中心距離 NMS。框取核心外接框（GT ≈ 1.03×核外接框，只加極小 padding）。用窄 hue 鎖定真紫色，連淡染核都抓得到；核心深染驗證確保整張抹片偏紫時不會把淡紫 RBC 團誤收成白血球；相對飽和度閘則避免偏紫染色在整張圖中標、合併成一個橫跨全圖的白血球框（那會把框內所有紅血球都排除掉）。**偵測完的 WBC 框會傳給 RBC 與血小板偵測器，排除中心落在 WBC 框內的候選**，白血球就不會被切成紅血球或血小板。

**RBC（`detect_rbc_rule`）**
取前處理後的二值圖 → `findContours`（`RETR_LIST`）抽輪廓 → 用**面積、邊界框尺寸、長寬比、圓形度**四道規則篩選（圓形度 `4πA/P²`），並排除落在紫色區內的輪廓（避免把 WBC 算成 RBC）→ **包含抑制**（當過大團塊框內已有更細的單顆框中心，丟掉該團塊框，避免一顆細胞被框兩次）→ 中心距離 NMS。`RETR_LIST` 的內孔輪廓本身即可把黏連團塊切成單顆（保留召回）；因 RBC 大小極一致（GT 邊長 ≈ 2·r0），框半徑往 `r0` 收斂、再 **×1.13 校正**（暗環輪廓比 GT 框小 ~11%，補回後 IoU 0.5 明顯提升）以提升 IoU。

**血小板（`detect_platelets_rule`，純規則）**
寬鬆紫色遮罩取小面積連通元件 → 抽取手工特徵 → 用**顆粒紋理**（飽和度變異 `S_std`、灰階變異 `gray_std`）作為主判據，配合顏色（`S_mean`、`B_mean`）與面積門檻 → 排除中心落在**加 0.22·r0 margin 的 WBC 框**內的候選（去掉白血球邊緣的假血小板）。真血小板內部有顆粒造成明顯亮度起伏，背景紫色雜訊則均勻，因此紋理是最乾淨的分界（雜訊預測由 1617 降到 ~21）。框設成固定大小以對齊一致的血小板 GT。
...
| --- | --- | --- | --- | --- |
| WBC | 0.95 | 0.93 | 0.93 | −3.0% |
| RBC | 0.78 | 0.78 | 0.78 | +7.5% |
| Platelet | 0.72 | 0.73 | 0.70 | −6.1% |
| **micro** | **0.79** | **0.78** | **0.79** | — |

WBC 在 IoU 0.3 下 F1 達 ~0.93（精確率 0.94–0.96），RBC 召回率 ~0.81、F1 ~0.78，三類細胞計數誤差皆在 **±30%** 內，且 test/val/全資料集一致（未過擬合）。經框大小校正後 **IoU 0.5 下 micro F1 亦達 ~0.76**（WBC 0.87、RBC 0.75），0.3 與 0.5 落差僅 ~0.03，框不靠寬鬆閾值也夠準。完整優化歷程（含偏紫抹片的整張圖白血球框修正、框大小校正）見 [`report.md`](report.md)。
