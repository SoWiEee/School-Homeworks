...
WBC 在 IoU 0.3 下 F1 達 ~0.93（精確率 0.94–0.96），RBC 經 Hough 圓偵測補強與自適應紫色排除後召回率 ~0.87、F1 ~0.80，三類細胞計數誤差皆在 **±30%** 內，且 test/val/全資料集一致（未過擬合）。經框大小校正後 **IoU 0.5 下 micro F1 亦達 ~0.78**（WBC 0.87、RBC 0.78），0.3 與 0.5 落差僅 ~0.03，框不靠寬鬆閾值也夠準。完整優化歷程（含偏紫抹片白血球框、框大小校正、Watershed 前景修正、Hough 相連細胞補強與自適應紫色排除）見 [`report.md`](report.md)。
